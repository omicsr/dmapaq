...
phenotypes %>% 
  ...
  dplyr::summarise(
    "Bad Samples (N)" = sum(call_rate < `< Call Rate Threshold`),
    "Bad Samples (%)" = signif(sum(call_rate < `< Call Rate Threshold`) / dplyr::n() * 100, digits = 3)
  ) %>%
  ...
  gt::tab_style(
    ...
    locations = gt::cells_body(
      columns = gt::vars(`< Call Rate Threshold`, `Bad Samples (N)`, `Bad Samples (%)`), 
...
phenotypes <- phenotypes %>% 
  ...
  dplyr::mutate(
    ...
    qc_gender_discrepancy = is.na(qc_observed_gender) | qc_observed_gender != qc_predicted_gender
  )
```
...
if (any(phenotypes[["qc_gender_discrepancy"]])) {
  phenotypes %>% 
