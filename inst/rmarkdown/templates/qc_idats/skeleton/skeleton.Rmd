...
for (ibeta in seq_along(list_beta)) {
  ...
  pca_dfxy[, 
    ...
    is_outlier := factor(
      x = dist_centre > (
        stats::quantile(dist_centre, 0.75, na.rm = TRUE) + params[["pca_outliers"]] * stats::IQR(dist_centre, na.rm = TRUE)
      ) |
        dist_centre < (
          stats::quantile(dist_centre, 0.25, na.rm = TRUE) - params[["pca_outliers"]] * stats::IQR(dist_centre, na.rm = TRUE)
        ),
      levels = c(FALSE, TRUE), 
      labels = c("No", "Yes")
    )
  ...
  p <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      ...
      caption = paste0(
        ...
        "&bull; higher than ", params[["pca_outliers"]], " times the interquartile range above the 75<sup>th</sup> percentile.<br>",
        "&bull; lower than ", params[["pca_outliers"]], " times the interquartile range below the 25<sup>th</sup> percentile.<br>",
