...
source('~/disks/PROJECT/_Mickael/PACKAGES/dmapaq/R/read_idats.R')
...
sample_sheet[, Sample_ID := as.chracter(Sample_Name)]
...
cat(data_idats[["log"]], sep = "\n")
data_mset <- data_idats[["mset"]]
phenotypes <- as.data.table(data_mset@metadata[["phenotypes"]])[, Sample_ID := as.character(Sample_ID)]
...
ggplot2::ggplot(
  ...
  mapping = ggplot2::aes(x = seq_along(call_rate), y = call_rate)
) +
  ggplot2::geom_point(
...
```{r cell_composition, eval = do_cell_composition, include = FALSE}
cell_comp <- switch(
...
p_heatmap <- list(
  ggplot2::ggplot(
    ...
    mapping = ggplot2::aes(x = variable, y = Sample_ID, fill = scales::rescale(value, to = c(0, 1)))
  ) +
    ggplot2::geom_tile() +
...
```{r sex_check_computation, eval = do_check_sex, include = FALSE}
...
ggplot2::ggplot(data = phenotypes, mapping = ggplot2::aes(x = qc_ymedian - qc_xmedian)) +
...
data.table::fwrite(
  x = data.table::as.data.table(norm_beta)[, cpg_id := rownames(norm_beta)],
...
for (ibeta in seq_along(list_beta)) {
  ...
  pca_dfxy <- data.table::as.data.table(pca_res[["projection"]], keep.rownames = "Sample_ID")
  data.table::setnames(
    x = pca_dfxy, 
  ...
  p_inertia <- ggplot2::ggplot(
    ...
    mapping = ggplot2::aes(x = paste0(x, " (", percent2(y), ")"), y = y)
  ) +
    ggplot2::geom_col(width = 1, colour = "white", fill = scales::viridis_pal(begin = 0.5, end = 0.5)(1), na.rm = TRUE) +
    ggplot2::scale_y_continuous(labels = percent1, expand = ggplot2::expansion(mult = c(0, 0.05))) +
    ggplot2::labs(
  ...
  if (length(keep_technical) > 0) {
    ...
    p_association <- ggplot2::ggplot(
      data = data.table::melt(
        ...
        variable.name = "pc", 
        value.name = "values"
      )[pc %in% sprintf("PC%02d", 1:fig_n_comp)][,
        data.table::as.data.table(stats::anova(stats::lm(
          formula = stats::as.formula(paste0("values ~ ", paste(keep_technical, collapse = " + "))),
          data = .SD
        )), keep.rownames = "term")[term != "Residuals"],
        by = "pc"
      ],
      ...
      ggtext::geom_richtext(
        mapping = ggplot2::aes(label = scientific(`Pr(>F)`, digits = 2)),
        colour = "white",
        size = 3,
        na.rm = TRUE
      ) +
      ...
      ggplot2::scale_x_discrete(
        ...
        labels = function(x) {
          paste0(
            x, " (",
            percent2((pca_res$values / sum(pca_res$values))[as.numeric(gsub("PC", "", x))]), 
            ")"
          )
        }
      ) +
  ...
  pca_dfxy[, 
    ...
    is_outlier := factor(
      x = dist_centre > (
        stats::quantile(dist_centre, 0.75, na.rm = TRUE) + 1.5 * stats::IQR(dist_centre, na.rm = TRUE)
      ) |
        dist_centre < (
          stats::quantile(dist_centre, 0.25, na.rm = TRUE) - 1.5 * stats::IQR(dist_centre, na.rm = TRUE)
        ),
      levels = c(FALSE, TRUE), 
      labels = c("No", "Yes")
    )
  ...
  p <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      title = paste0("Outliers Detection In Factorial Planes"),
      caption = paste0("Contribution computed using ", n_comp," principal components."),
      tag_levels = "A"
    )
  print(p)
