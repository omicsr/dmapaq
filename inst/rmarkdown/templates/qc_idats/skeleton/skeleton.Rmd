  ...
  if (length(keep_technical) > 0) {
    ...
    p_association <- ggplot2::ggplot(
      data = data.table::melt(
        ...
        data.table::rbindlist(lapply(X = keep_technical, .data = .SD, FUN = function(.x, .data) {
          as.data.table(
            stats::anova(stats::lm(formula = as.formula(paste0("values ~ ", .x)), data = .SD)), 
            keep.rownames = "term"
          )[term != "Residuals"]
        })),
        by = "pc"
      ...
      ggplot2::scale_x_discrete(
        ...
        labels = function(x) {
          paste0(
            ...
            scales::percent_format(accuracy = 0.01, suffix = " %")(pca_res[["pve"]][as.numeric(gsub("PC", "", x))]), 
            ")</i>"
          )
        }
      ) +
  ...
  p <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      ...
      caption = paste0(
        ...
        "higher than ", params[["pca_threshold"]], " times the interquartile range above the 75<sup>th</sup> percentile."
      ),
      tag_levels = "A", 
