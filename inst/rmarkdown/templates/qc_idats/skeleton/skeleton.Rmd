...
if (
  (
    !grepl("FlowSorted.blood", params[["cell_tissue"]], ignore.case = TRUE) &
      !grepl("FlowSorted.cordblood", params[["cell_tissue"]], ignore.case = TRUE)
  ) &
...
if (length(unique(minfi::pData(data_mset)[["Sentrix_ID"]])) > 1) { 
  norm_beta <- do.call("rbind", parallel::mclapply(
    X = split(
      x = as.data.frame(norm_beta[sample(1:nrow(norm_beta)), ]), 
      f = (1:nrow(norm_beta)-1) %/% ( (nrow(norm_beta)-1) %/% params[["n_cores"]] )
    ), 
    .batch = factor(minfi::pData(data_mset)[["Sentrix_ID"]]), 
    mc.cores = params[["n_cores"]],
    FUN = function(x, .batch) {
      sva::ComBat(dat = as.matrix(x), batch = .batch)
    }
  ))
  norm_beta <- norm_beta[rownames(data_mset), ]
}
